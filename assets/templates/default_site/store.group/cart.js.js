...
function update_cart(data, statusText, xhr, $form)  {   
    if (data.success) {                                                   
		...
		// update the XID hash so we don't run afoul of EE's secure forms
		jQuery("input[name=XID]").val(data.XID);
...
jQuery(document).ready(function($){
	...
	$("#gateway").live('change', function(){
 		$.post("{path=store/ajax_cart_form/}", { gateway: $(this).val(), XID : $(this).closest("form").find("input[name=XID]").val()  },
			function(data) {
				// $(this).closest("form").find("input[name=XID]").val(data.XID); 
