...
starter.controller("SendgiftCtrl", function($scope, $http, $rootScope, $ionicLoading, $ionicPopup, $window, $ionicViewService, $location, $ionicModal, $state) {
        ...
        $scope.submitFormBottleNow = function ( item_id, estId, senderId, recipientId, method ) {
        ...
        $scope.submitFormBottle = function (senderId) {

            $http.get(config.template_path + '/username_to_id/' + $scope.invite.username).success(function(result) {
                if(!result.userid) {
                  $rootScope.hideLoading();
                  $scope.showError("Send Gift Error", "Sorry, that username was not found.");
                } else {               	
...
starter.controller('estBottleController', function($scope, $stateParams, $rootScope, $http, $ionicLoading, $ionicModal, $ionicScrollDelegate, $timeout, $ionicPopup, $state, $window) {
  ...
  $scope.queryEstablishments = function() {
  	...
  	//$scope.showAlert( '', 'queryEstablishments' );
    ...
    $http.get(config.template_path + '/estjson/'+$rootScope.location.latitude+'/'+$rootScope.location.longitude).success(function(results) {
     ...
     //   for ( var i = 0; i <  $rootScope.establishments.length; i++) {   	                    	
     //   	$scope.showAlert( '',  $rootScope.establishments[i].title );
     //   }
  ...
  $scope.showEstMenu = function(estId, sender, recipient, method ) {
    ...
    $state.transitionTo('app.menuBottle', {estId: estId, sender: sender, recipient : recipient, method : method } );
...
starter.controller('SendbottleCtrl', function($scope, $stateParams, $ionicPopup, $http, $window, $ionicModal, $ionicLoading, $rootScope, $state) {
      ...
      $scope.sendBottleByContactsText = function( memId ) {
        $rootScope.showLoading();
      ...
      $scope.contactInfoPhone = function(r) {      	
    	$state.transitionTo('app.establishments_bottle', {senderId: $scope.senderId, recipientid: r.phone, method : "text"});
       },
